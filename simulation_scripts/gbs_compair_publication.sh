#!/bin/bash
workingdir="/tmp_file";
gbsdir="/gbs_installation_dir";
stacksdir="/stacks_installation_dir/bin";
javadir="/usr/bin";
cd $workingdir;
...
do
    ...
    awk '{OUT="barcode.stacks." length($0) ".txt";}; OUT{print >OUT}' barcode_list.txt;
    awk '{print $1 "\t" $1 "\tApeKI"}' barcode_list.txt > barcode.gbsx.txt;
    awk '{print $1 "\t" $1 "\tNAN"}' barcode_list.txt > barcode.nan.txt;

    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --GBSsimulator -f $workingdir/chr21.out.fa -p $paired -l $readlength -d $depth -b barcode_list.txt;
	
	
	totalCounts=`cat output.R1.fastq | wc -l`;
    totalReads=`expr $totalCounts / 4`;
    readCounts=`expr $totalReads / $barcodeNr`;
    longestBarcodeLength=`awk 'BEGIN{longest=0;}{if (length($1) > longest){longest=length($1);};}END{printf longest;}' barcode_list.txt`;
...
##### GBSX GBS paired end simulation 
    ...
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -f2 output.R2.fastq -i barcode.gbsx.txt -o GBSX_GBS_pe;
...
##### GBSX RAD paired end simulation
    ...
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -f2 output.R2.fastq -i barcode.gbsx.txt -o GBSX_RAD_pe -rad true;
...
##### GBSX inline barcode paired end simulation
    ...
    echo -n "[COMPAIR] GBSX_NAN start: "
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -f2 output.R2.fastq -i barcode.nan.txt -o GBSX_NAN_pe -rad true;
    echo -n "[COMPAIR] GBSX_NAN end: "
    date;

    for i in `ls GBSX_NAN_pe/*.R1.fastq`;
    do
        if [[ $i != *undetermined* ]]
        then
            barcode=`echo $i | sed 's/GBSX\_NAN\_pe\///g' | sed 's/\.R1\.fastq//g'`;
            perl $workingdir/compair_demultiplex.pl $workingdir/chr21.out.fa $i $trimlengths $barcode >> GBSX_NAN_pe.sum.tsv;
            echo $readCounts >> GBSX_NAN_pe.sum.tsv;
        fi
    done
...
#### Stacks RAD paired end simulation
    ...
    date;
    for i in `ls barcode.stacks.*.txt`;
    do
        $stacksdir/process_radtags -1 output.R1.fastq -2 output.R2.fastq -o stacks_radtags_pe/ -e apeKI -b $i --adapter_1 AGATCGGAAGAGCG --adapter_2 AGATCGGAAGAGCG --adapter_mm 1 --inline_null -s 0;
    done
...
##### Stacks inline barcode paired end simulation
    ...
    for i in `ls barcode.stacks.*.txt`;
    do
        $stacksdir/process_shortreads -1 output.R1.fastq -2 output.R2.fastq -o stacks_shortreads_pe/ -b $i --adapter_1 AGATCGGAAGAGCG --adapter_2 AGATCGGAAGAGCG --adapter_mm 1 --inline_null -s 0;
    done
...
##### GBSX GBS signle read simulation
    ...
    echo -n "[COMPAIR] GBSX_GBS start: "
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -i barcode.gbsx.txt -o GBSX_GBS_sr;
    echo -n "[COMPAIR] GBSX_GBS end: "
    date;
...
##### GBSX RAD signle read simulation
    ...
    echo -n "[COMPAIR] GBSX_RAD start: "
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -i barcode.gbsx.txt -o GBSX_RAD_sr -rad true;
    echo -n "[COMPAIR] GBSX_RAD end: "
    date;
...
##### GBSX inline barcode signle read simulation
    ...
    echo -n "[COMPAIR] GBSX_NAN start: "
    date;
    $javadir/java -jar $gbsdir/GBSX_v1.0.jar --Demultiplexer -f1 output.R1.fastq -i barcode.nan.txt -o GBSX_NAN_sr -rad true;
    echo -n "[COMPAIR] GBSX_NAN end: "
    date;

    for i in `ls GBSX_NAN_sr/*.R1.fastq`;
    do
        if [[ $i != *undetermined* ]]
        then
            barcode=`echo $i | sed 's/GBSX\_NAN\_sr\///g' | sed 's/\.R1\.fastq//g'`;
            perl $workingdir/compair_demultiplex.pl $workingdir/chr21.out.fa $i $trimlengths $barcode >> GBSX_NAN_sr.sum.tsv;
            echo $readCounts >> GBSX_NAN_sr.sum.tsv;
        fi
    done
...
##### Stacks RAD signle read simulation
    ...
    for i in `ls barcode.stacks.*.txt`;
    do
        $stacksdir/process_radtags -f output.R1.fastq -o stacks_radtags_sr/ -e apeKI -b $i --adapter_1 AGATCGGAAGAGCG --adapter_2 AGATCGGAAGAGCG --adapter_mm 1 --inline_null -s 0;
    done
    echo -n "[COMPAIR] stacks_radtags end: "
    date;
...
##### Stacks inline barcode signle read simulation
    ...
    for i in `ls barcode.stacks.*.txt`;
    do
        $stacksdir/process_shortreads -f output.R1.fastq -o stacks_shortreads_sr/ -b $i --adapter_1 AGATCGGAAGAGCG --adapter_2 AGATCGGAAGAGCG --adapter_mm 1 --inline_null -s 0;
    done
    echo -n "[COMPAIR] stacks_shortreads end: "
    date;
...
###### Remove all simulated files
    rm output.R1.fastq_sr;
    rm output.R2.fastq_sr;
...
do
	...
	cat $workingdir/test_$counter/GBSX_NAN_pe.sum.tsv >> GBSX_NAN_pe.sum.tsv;
	...
	cat $workingdir/test_$counter/GBSX_NAN_sr.sum.tsv >> GBSX_NAN_sr.sum.tsv;
...
awk 'BEGIN{counter=1;} {if ($2 == 0 || $5 == 0){print counter"\t"0;}else{print counter"\t"($2/$5);} counter++;}' GBSX_NAN_pe.sum.tsv > GBSX_NAN_pe.cor_all.tsv;
awk 'BEGIN{counter=1;} {if ($3 == 0 || $5 == 0){print counter"\t"0;}else{print counter"\t"($3/$5);} counter++;}' GBSX_NAN_pe.sum.tsv > GBSX_NAN_pe.dem_all.tsv;
awk 'BEGIN{counter=1;} {if ($3 == 0 || $4 == 0){print counter"\t"0;}else{print counter"\t"($4/$3);} counter++;}' GBSX_NAN_pe.sum.tsv > GBSX_NAN_pe.fault_dem.tsv;
...
join -j 1 GBSX_GBS_RAD_pe.cor_all.tsv GBSX_NAN_pe.cor_all.tsv > GBSX_pe.cor_all.tsv;
join -j 1 stacks_radtags_pe.cor_all.tsv stacks_shortreads_pe.cor_all.tsv > stacks_pe.cor_all.tsv;
join -j 1 GBSX_pe.cor_all.tsv stacks_pe.cor_all.tsv > all_pe.cor_all.tsv;
rm GBSX_GBS_RAD_pe.cor_all.tsv; rm GBSX_pe.cor_all.tsv; rm stacks_pe.cor_all.tsv;

join -j 1 GBSX_GBS_pe.dem_all.tsv GBSX_RAD_pe.dem_all.tsv > GBSX_GBS_RAD_pe.dem_all.tsv;
join -j 1 GBSX_GBS_RAD_pe.dem_all.tsv GBSX_NAN_pe.dem_all.tsv > GBSX_pe.dem_all.tsv;
join -j 1 stacks_radtags_pe.dem_all.tsv stacks_shortreads_pe.dem_all.tsv > stacks_pe.dem_all.tsv;
join -j 1 GBSX_pe.dem_all.tsv stacks_pe.dem_all.tsv > all_pe.dem_all.tsv;
rm  GBSX_GBS_RAD_pe.dem_all.tsv; rm GBSX_pe.dem_all.tsv; rm stacks_pe.dem_all.tsv;

join -j 1 GBSX_GBS_pe.fault_dem.tsv GBSX_RAD_pe.fault_dem.tsv > GBSX_GBS_RAD_pe.fault_dem.tsv;
join -j 1 GBSX_GBS_RAD_pe.fault_dem.tsv GBSX_NAN_pe.fault_dem.tsv > GBSX_pe.fault_dem.tsv;
join -j 1 stacks_radtags_pe.fault_dem.tsv stacks_shortreads_pe.fault_dem.tsv > stacks_pe.fault_dem.tsv;
join -j 1 GBSX_pe.fault_dem.tsv stacks_pe.fault_dem.tsv > all_pe.fault_dem.tsv;
rm GBSX_GBS_RAD_pe.fault_dem.tsv; rm GBSX.fault_dem_pe.tsv; rm stacks_pe.fault_dem.tsv;
...
awk 'BEGIN{counter=1;} {if ($2 == 0 || $5 == 0){print counter"\t"0;}else{print counter"\t"($2/$5);} counter++;}' GBSX_NAN_sr.sum.tsv > GBSX_NAN_sr.cor_all.tsv;
awk 'BEGIN{counter=1;} {if ($3 == 0 || $5 == 0){print counter"\t"0;}else{print counter"\t"($3/$5);} counter++;}' GBSX_NAN_sr.sum.tsv > GBSX_NAN_sr.dem_all.tsv;
awk 'BEGIN{counter=1;} {if ($3 == 0 || $4 == 0){print counter"\t"0;}else{print counter"\t"($4/$3);} counter++;}' GBSX_NAN_sr.sum.tsv > GBSX_NAN_sr.fault_dem.tsv;
...
join -j 1 GBSX_GBS_RAD_sr.cor_all.tsv GBSX_NAN_sr.cor_all.tsv > GBSX_sr.cor_all.tsv;
join -j 1 stacks_radtags_sr.cor_all.tsv stacks_shortreads_sr.cor_all.tsv > stacks_sr.cor_all.tsv;
join -j 1 GBSX_sr.cor_all.tsv stacks_sr.cor_all.tsv > all_sr.cor_all.tsv;
rm GBSX_GBS_RAD_sr.cor_all.tsv; rm GBSX_sr.cor_all.tsv; rm stacks_sr.cor_all.tsv;

join -j 1 GBSX_GBS_sr.dem_all.tsv GBSX_RAD_sr.dem_all.tsv > GBSX_GBS_RAD_sr.dem_all.tsv;
join -j 1 GBSX_GBS_RAD_sr.dem_all.tsv GBSX_NAN_sr.dem_all.tsv > GBSX_sr.dem_all.tsv;
join -j 1 stacks_radtags_sr.dem_all.tsv stacks_shortreads_sr.dem_all.tsv > stacks_sr.dem_all.tsv;
join -j 1 GBSX_sr.dem_all.tsv stacks_sr.dem_all.tsv > all_sr.dem_all.tsv;
rm  GBSX_GBS_RAD_sr.dem_all.tsv; rm GBSX_sr.dem_all.tsv; rm stacks_sr.dem_all.tsv;

join -j 1 GBSX_GBS_sr.fault_dem.tsv GBSX_RAD_sr.fault_dem.tsv > GBSX_GBS_RAD_sr.fault_dem.tsv;
join -j 1 GBSX_GBS_RAD_sr.fault_dem.tsv GBSX_NAN_sr.fault_dem.tsv > GBSX_sr.fault_dem.tsv;
join -j 1 stacks_radtags_sr.fault_dem.tsv stacks_shortreads_sr.fault_dem.tsv > stacks_sr.fault_dem.tsv;
join -j 1 GBSX_sr.fault_dem.tsv stacks_sr.fault_dem.tsv > all_sr.fault_dem.tsv;
rm GBSX_GBS_RAD_sr.fault_dem.tsv; rm GBSX.fault_dem_sr.tsv; rm stacks_sr.fault_dem.tsv;
